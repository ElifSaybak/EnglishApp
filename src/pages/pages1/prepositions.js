...
*/
  const addWord = async () => {
    if (!eng || !turk) return;
    try {
      const response = addDoc(collection(firestore, "prepositions"), {
        english: eng,
        turkish: turk,
      });
  ...
  const readFirebase = async () => {
    try {
      const get = await getDocs(collection(firestore, "prepositions"));
      setFireData([]);
      get.forEach((doc) => {
        console.log(
    ...
    <View style={styles.container}>
      ...
      <Provider>
        <Portal>
          ...
          >
            <Dialog.Title style={{ color: "#ff8912", fontWeight: "bold" }}>
              New Preposition
            </Dialog.Title>
            <Dialog.Content>
              <TextInput
            ...
            <Dialog.Actions>
              <Button
                style={{ backgroundColor: "#ff8912", marginRight: 5 }}
                onPress={() => setIsDialogVisible(false)}
              >
                Cancel
              </Button>
              <Button
                style={{ backgroundColor: "#ff8912", marginRight: 15 }}
                onPress={() => {
