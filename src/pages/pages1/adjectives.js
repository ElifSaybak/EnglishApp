...
*/
  const addWord = async () => {
    if (!eng || !turk) return;
    try {
      const response = addDoc(collection(firestore, "adjectives"), {
        english: eng,
        turkish: turk,
      });
  ...
  const readFirebase = async () => {
    try {
      const get = await getDocs(collection(firestore, "adjectives"));
      setFireData([]);
      get.forEach((doc) => {
        console.log(
    ...
    <View style={styles.container}>
      ...
      <Provider>
        <Portal>
          ...
          >
            <Dialog.Title style={{ color: "#ff5050", fontWeight: "bold" }}>
              New Adjective
            </Dialog.Title>
            <Dialog.Content>
              <TextInput
            ...
            <Dialog.Actions>
              <Button
                style={{ backgroundColor: "#ff5050", marginRight: 5 }}
                onPress={() => setIsDialogVisible(false)}
              >
                Cancel
              </Button>
              <Button
                style={{ backgroundColor: "#ff5050", marginRight: 15 }}
                onPress={() => {
